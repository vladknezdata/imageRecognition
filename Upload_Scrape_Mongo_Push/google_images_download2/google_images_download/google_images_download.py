...
class googleimagesdownload:
            ...
            try:
                ...
                if format:
                    if not image_format or image_format != format:
                        ...
                        absolute_path = ''
                        image_url = ''
                        return download_status, download_message, return_image_name, absolute_path, image_url
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        return items,errorCount,abs_path,image_urls
    ...
    def download(self,arguments):
        ...
        if __name__ != "__main__":
            ...
            if 'config_file' in arguments:
                ...
                total_errors = 0
                for rec in records:
                    paths, errors, image_urls = self.download_executor(rec)
                    for i in paths:
                        paths_agg[i] = paths[i]
                    if not arguments["silent_mode"]:
                ...
                return paths_agg,total_errors, image_urls
            # if the calling file contains params directly
            else:
                paths, errors, image_urls = self.download_executor(arguments)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
                    if arguments['print_paths']:
                        print(paths.encode('raw_unicode_escape').decode('utf-8'))
                return paths_agg, errors, image_urls
    ...
    def download_executor(self,arguments):
        ...
        for pky in prefix_keywords:                 # 1.for every prefix keywords
            for sky in suffix_keywords:             # 2.for every suffix keywords
                ...
                while i < len(search_keyword):      # 3.for every main keyword
                    ...
                    items,errorCount,abs_path,image_url = self._get_all_items(raw_html,main_directory,dir_name,limit,arguments)    #get all image items and download images
                    paths[pky + search_keyword[i] + sky] = abs_path
                    image_urls[pky + search_keyword[i] + sky] = image_url
        ...
        return paths, total_errors, image_urls
...
#             response = googleimagesdownload()
#             paths,errors,image_urls = response.download(arguments)  #wrapping response in a variable just for consistency
#             total_errors = total_errors + errors
